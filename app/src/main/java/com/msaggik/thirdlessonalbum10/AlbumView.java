...
public class AlbumView extends View {
    ...
    private float[] startCords = new float[2], endCords = new float[2]; //x, y
        ...
        if (figure.equals("line")) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    startCords[0] = event.getX();
                    startCords[1] = event.getY();
                    break;
                case MotionEvent.ACTION_UP:
                    endCords[0] = event.getX();
                    endCords[1] = event.getY();

                    canvas.drawLine(startCords[0], startCords[1], endCords[0], endCords[1], paint);
            }
        }
